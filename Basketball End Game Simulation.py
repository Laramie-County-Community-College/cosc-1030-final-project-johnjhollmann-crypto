    ...
    if strategy == "3_pointer":
        ...
        if has_ball and current_time >= TIME_PER_3PT_POSSESSION:
            ...
            if random.random() < params['my_3pt_pct']:
                ...
                if current_time >= TIME_PER_OPP_POSSESSION:
                    ...
                    # Not enough time for opponent possession, game is over
                    pass
            else:
        ...
        if current_score_diff < 0:
            while current_time > 0 and current_score_diff < 0:
                ...
                # Opponent shoots first free throw (no time off the clock)
                if random.random() < params['opp_ft_pct']:
                    current_score_diff -= 1
                
                # Opponent shoots second free throw (no time off the clock)
                if random.random() < params['opp_ft_pct']:
                    current_score_diff -= 1
                else:
                    ...
                    if random.random() < (1 - params['ft_rebound_pct']):
                        has_ball = True
                        # Clock starts on the rebound, then a new possession begins
                        current_time -= TIME_PER_REBOUND_AND_SHOT
                    else:
...
if __name__ == "__main__":
    ...
    simulation_params = {
        ...
        'ft_rebound_pct': 0.20,
        'ot_win_pct': 0.50,
        'opp_poss_score_pct': 0.50, #Opponent scoring% after a made 3
    }
